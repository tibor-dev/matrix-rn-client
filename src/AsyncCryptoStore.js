...
function keyEndToEndSession(deviceKey, sessionKey) {
    return E2E_PREFIX + 'sessions/' + encodeURIComponent(deviceKey) + '/' + encodeURIComponent(sessionKey);
}
...
function keyEndToEndSessionProblems(deviceKey, sessionKey) {
    return E2E_PREFIX + 'session.problems/' + encodeURIComponent(deviceKey) + '/' + encodeURIComponent(sessionKey);
}

function keyEndToEndInboundGroupSession(senderKey, sessionId) {
    return KEY_INBOUND_SESSION_PREFIX + encodeURIComponent(senderKey) + '/' + encodeURIComponent(sessionId);
}

function keyEndToEndInboundGroupSessionWithheld(senderKey, sessionId) {
    return KEY_INBOUND_SESSION_WITHHELD_PREFIX + encodeURIComponent(senderKey) + '/' + encodeURIComponent(sessionId);
}
 ...
 */
class AsyncCryptoStore extends sdk.MemoryCryptoStore {
    ...
    getEndToEndInboundGroupSession(senderCurve25519Key, sessionId, txn, func) {
        txn.wrap(async () => {
            func(
                ...
                await this._getJsonItem(keyEndToEndInboundGroupSessionWithheld(senderCurve25519Key, sessionId))
            );
        });
    }
    ...
    getAllEndToEndInboundGroupSessions(txn, func) {
        txn.wrap(async () => {
            const inboundGroupSessionKeys = (await this.asyncStorage.getAllKeys()).filter((k) => k.startsWith(KEY_INBOUND_SESSION_PREFIX));
    ...
    storeEndToEndInboundGroupSession(senderCurve25519Key, sessionId, sessionData, txn) {
        txn.wrap(() => {
            return this._setJsonItem(keyEndToEndInboundGroupSession(senderCurve25519Key, sessionId), sessionData);
        });
    }

    storeEndToEndInboundGroupSessionWithheld(senderCurve25519Key, sessionId, sessionData, txn) {
        txn.wrap(() => {
            return this._setJsonItem(keyEndToEndInboundGroupSessionWithheld(senderCurve25519Key, sessionId), sessionData);
        });
    }
    ...
    async getSessionsNeedingBackup(limit) {
        ...
        for (const k of Object.keys(sessionsNeedingBackup)) {
            ...
            const sessionData = await this._getJsonItem(keyEndToEndInboundGroupSession(senderKey, sessionId));
            sessions.push({
                senderKey: senderKey,
                sessionId: sessionId,
    ...
    async getOutgoingRoomKeyRequest(requestBody) {
        ...
        for (const reqId of allRequestIds) {
            ...
            if (req.requestBody.room_id === requestBody.room_id && req.requestBody.session_id === requestBody.session_id) {
                return req;
            }
        }
        return null;
